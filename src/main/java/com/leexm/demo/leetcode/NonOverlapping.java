...
public class NonOverlapping {
    ...
    public static int eraseOverlapIntervals(int[][] intervals) {
        ...
        // sort2(intervals, 0, intervals.length - 1);
        intervals = Arrays.stream(intervals).sorted(Comparator.comparingInt(o -> o[0])).sorted(Comparator.comparingInt(o -> o[1]))
            .toArray(int[][]::new);
        ...
        remove.forEach(item -> System.out.println(Arrays.toString(item)));
        return remove.size();
    }
    ...
    private static void sort(int[][] intervals) {
        for (int i = intervals.length - 1; i >= 0; i--) {
            for (int j = 0; j < i; j++) {
                int[] t = intervals[j];
                int[] t1 = intervals[j + 1];
                if ((t[1] - t1[1]) > 0) {
                    intervals[j] = t1;
                    intervals[j + 1] = t;
                } else if ((t[1] - t1[1]) == 0 && (t[0] - t1[0]) > 0) {
                    intervals[j] = t1;
                    intervals[j + 1] = t;
                }
            }
